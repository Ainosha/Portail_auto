...
int notes = sizeof(melody_zelda) / sizeof(melody_zelda[0]) / 2;
...
void song_zeldalullaby(int buzzerPin) 
{
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
    ...
    divider = melody_zelda[thisNote + 1];
    if (divider > 0) {
    ...
    tone(buzzerPin, melody_zelda[thisNote], noteDuration*0.9);
...
void coffin_dance_song(int buzzerPin) 
{
...
// notes_mii of the moledy followed by the duration.
// a 4 means a quarter note, 8 an eighteenth , 16 sixteenth, so on
// !!negative numbers are used to represent dotted notes_mii,
// so -4 means a dotted quarter note, that is, a quarter plus an eighteenth!!
int melody_mii[] = {
...
// sizeof gives the number of bytes, each int value is composed of two bytes (16 bits)
// there are two values per note (pitch and duration), so for each note there are four bytes
int notes_mii = sizeof(melody_mii) / sizeof(melody_mii[0]) / 2;

// this calculates the duration of a whole note in ms
int wholenote_mii = (60000 * 4) / tempo_mii;

int divider_mii = 0, noteDuration_mii = 0;

void song_mii(int buzzerPin) 
{
  // iterate over the notes_mii of the melody_mii. 
  // Remember, the array is twice the number of notes_mii (notes_mii + durations)
  for (int thisNote = 0; thisNote < notes_mii * 2; thisNote = thisNote + 2) {

    // calculates the duration of each note
    divider_mii = melody_mii[thisNote + 1];
    if (divider_mii > 0) {
      // regular note, just proceed
      noteDuration_mii = (wholenote_mii) / divider_mii;
    } else if (divider_mii < 0) {
      // dotted notes_mii are represented with negative durations!!
      noteDuration_mii = (wholenote_mii) / abs(divider_mii);
      noteDuration_mii *= 1.5; // increases the duration in half for dotted notes_mii
    }

    // we only play the note for 90% of the duration, leaving 10% as a pause
    tone(buzzerPin, melody_mii[thisNote], noteDuration_mii*0.9);

    // Wait for the specief duration before playing the next note.
    delay(noteDuration_mii);
